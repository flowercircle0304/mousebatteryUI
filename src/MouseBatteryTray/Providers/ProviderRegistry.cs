    ...
    public static IReadOnlyList<IMouseBatteryProvider> BuildAll(AppSettings settings)
    {
        ...
        foreach (var d in settings.DiscoveredDevices)
        {
            IMouseBatteryProvider? provider = d.Kind switch
            {
                ...
                "wlmouse-strider" => new WlMouseStriderProvider(d.Id, d.DisplayName, new[] { d.ProductId }.Concat(d.AdditionalProductIds), d.VendorId, (byte)d.MouseTarget),
                "endgame-gear-we" => new EndgameGearWeProvider(d.Id, d.DisplayName),
                "ninjutso" => new NinjutsoProvider(d.Id, d.DisplayName, new[] { d.ProductId }.Concat(d.AdditionalProductIds)),
                "finalmouse-ulx" => new FinalmouseProvider(d.Id, d.DisplayName),
                "pulsar" => new PulsarProvider(d.Id, d.DisplayName),
                "moddo-mouse" => new ModdoMouseProvider(d.Id, d.DisplayName),
